...
namespace Ticari_Otomasyon
{
    public partial class Frm_Personeller : Form
    {
        ...
        void listele()
        {
            ...
            gridView1.Columns["ID"].Caption = "PERSONEL Kodu";
        }
        void sehirler()
        {
        ...
        private void Btn_Kaydet_Click(object sender, EventArgs e)
        {
            try
            {
                SqlCommand cmd = new SqlCommand("insert into TBL_PERSONELLER (AD, SOYAD, TELEFON, TC, MAIL, IL, ILCE, ADRES, GOREV) values (@p1,@p2,@p3,@p5,@p6,@p7,@p8,@p9,@p10)", bgl.baglanti());
        ...
        private void Btn_Sil_Click(object sender, EventArgs e)
        {
            ...
            if (gridView1.GetFocusedRowCellValue("ID") != null)
            {
                ...
                DialogResult result = XtraMessageBox.Show(
                    "Seçilen personeli silmek istediğinize emin misiniz?",
                    "Onay",
                ...
                if (result == DialogResult.Yes)
                {
                    ...
                    try
                    {
                        SqlCommand cmd = new SqlCommand("DELETE FROM TBL_PERSONELLER WHERE ID = @ID", bgl.baglanti());
            ...
            {
                XtraMessageBox.Show("Lütfen silmek için bir personeli seçin.", "Uyarı", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        }
        ...
        private void Btn_guncelle_Click(object sender, EventArgs e)
        {
            try
            {
                SqlCommand cmd = new SqlCommand("Update TBL_PERSONELLER set AD=@p1, SOYAD=@p2, TELEFON=@p3, TC=@p5, MAIL=@p6, IL=@p7, ILCE=@p8, ADRES=@p9, GOREV=@p10 where ID=@p11", bgl.baglanti());
                cmd.Parameters.AddWithValue("@p1", Txt_Ad.Text);
                cmd.Parameters.AddWithValue("@p2", Txt_Soyad.Text);
                cmd.Parameters.AddWithValue("@p3", msk_Telefon1.Text);
                cmd.Parameters.AddWithValue("@p5", msk_TC.Text);
